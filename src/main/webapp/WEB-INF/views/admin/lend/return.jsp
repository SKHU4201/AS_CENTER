	...
	<div>
		<form method="post" name=myForm1>
			...
			<table class="return">
				<tr>
					<td colspan="3">대여시작일 :
						<fmt:formatDate pattern="yyyy-MM-dd" value="${ selectedLend.start_date }" />
					</td>
				</tr>
				<tr>
					<td colspan="3">반납예정일 :
						<fmt:formatDate pattern="yyyy-MM-dd" value="${ selectedLend.end_date }" />
					</td>
				</tr>
				<tr>
					<td colspan="3">반납일 :
				...
				<tr style="text-align: left;">
					<td>반납일연장: </td>
					<td>
						<fmt:formatDate pattern="yyyy-MM-dd" value="${ end_date }" var="b" /> <input type="date"
							class="form-control" name="end_date" value="${ b }" /></td>
				...
				<c:if test="${ returnday < endday }">
					<tr>
						<td colspan="3">반납예정일로부터 ${calDateDays}일 전입니다.</td>
					</tr>
				</c:if>
				<c:if test="${ returnday == endday }">
					<tr>
						<td colspan="3">반납예정일 당일입니다.</td>
					</tr>
				</c:if>
				<c:if test="${returnday > endday }">
					<tr>
						<td colspan="3">반납예정일로부터 ${calDateDays }일 지났습니다.</td>
					</tr>
					<tr>
						<td class="p">연체료: ${ arrears }원</td>
					</tr>
				</c:if>
		...
		<tr>
			<form method="post" name=myForm2>
				<td colspan="3" style="margin-left:300px;"><button type="button" class="btn btn-primary"
						onclick="javascript:closewin2()">반납</button></td>
				<td><input type="hidden" name=lend_no value="${lend_no}" /></td>
				<td><input type="hidden" name=arrears value="${arrears}" /></td>
		</tr>
		</form>
