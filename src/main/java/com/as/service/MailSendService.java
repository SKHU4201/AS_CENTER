...
public class MailSendService {
	...
	public String sendAuthMail(Member m) {
		...
		// String mailContent = "<h1>[이메일 인증]</h1><br><p>아래 링크를 클릭하시면 이메일 인증이
		// 완료됩니다.</p>"
		...
		emailcontent.append(" <div"
				...
				+ "	<p style=\"font-size: 16px; line-height: 26px; margin-top: 50px; padding: 0 5px;\">" + m.getName()
	...
	// 이메일 변경 링크를 메일로 보내기
	public void sendChangeEMail(Member m, String changeMail) {
		MimeMessage mail = mailSender.createMimeMessage();
		StringBuffer emailcontent = new StringBuffer();
		emailcontent.append("<!DOCTYPE html>");
		emailcontent.append("<html>");
		...
		emailcontent.append(" <div"
				...
				+ "	<h1 style=\"margin: 0; padding: 0 5px; font-size: 28px; font-weight: 400;\">"
				+ "		<span style=\"font-size: 15px; margin: 0 0 10px 3px;\">성공회대학교 A/S실 서비스</span><br />"
				+ "		<span style=\"color: #02b875\">이메일 변경 링크</span> 안내입니다." + "	</h1>\n"
				+ "	<p style=\"font-size: 16px; line-height: 26px; margin-top: 50px; padding: 0 5px;\">" + m.getName()
				+ "		님 안녕하세요.<br />" + "		성공회대학교 A/S실 서비스를 이용해 주셔서 진심으로 감사드립니다.<br />"
				+ "		아래 <b style=\"color: #02b875\">'이메일 변경'</b> 버튼을 클릭하여 이메일을 변경해주세요.<br />" + "		감사합니다."
				+ "	</p>" + "	<a style=\"color: #FFF; text-decoration: none; text-align: center;\""
				+ "	href=\"http://localhost:8088/user/mail/change_email?email=" + m.getEmail() + "&changeEMail=" + changeMail + "\" target=\"_blank\">" + "		<p"
				+ "			style=\"display: inline-block; width: 210px; height: 45px; margin: 30px 5px 40px; background: #02b875; line-height: 45px; vertical-align: middle; font-size: 16px;\">"
				+ "			이메일 변경</p>" + "	</a>" + "	<div style=\"border-top: 1px solid #DDD; padding: 5px;\"></div>"
				+ " </div>");
		emailcontent.append("</body>");
		emailcontent.append("</html>");
		...
		// 인증메일 보내기
		try {
			mail.setSubject("이메일 변경 인증 메일", "utf-8");
			mail.setText(emailcontent.toString(), "utf-8", "html");
			mail.addRecipient(Message.RecipientType.TO, new InternetAddress(m.getEmail()));
			mailSender.send(mail);
		} catch (MessagingException e) {
